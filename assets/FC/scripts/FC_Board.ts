...
export default class FC_Board {
    private constructor() {};
    private static instance: FC_Board = null;
    public static getInstance(): FC_Board{
        this.instance = this.instance || new FC_Board();
    ...
    private _initOuterRing(){
        for(let i = 0; i < FC_Board._ChessPointOutPosArr.length; i++){
            let p = FC_Board._ChessPointOutPosArr[i];       // 坐标
    ...
    private _initInnerRing(){
        for(let i = 0; i < FC_Board._ChessPointInPosArr.length; i++){
            let temp = FC_Board._ChessPointInPosArr[i];
            let dir = FC_Board._ChessPointInDirArr[i];      // 方向
    ...
    private _initWaitPoints(){
        for(let i = 0; i < FC_Board._ChessPointPlaneWaitPosArr.length; i++){
            let p = FC_Board._ChessPointPlaneWaitPosArr[i];     // 坐标
            let dir = FC_Board._ChessPointPlaneWaitDirArr[i];   // 方向
    ...
    private _initStopPoints(){
        for(let i = 0; i < FC_Board._ChessPointPlaneStopPosArr.length; i++){
            let temp = FC_Board._ChessPointPlaneStopPosArr[i];
            let dir = FC_Board._ChessPointPlaneWaitDirArr[i];       // 方向
    ...
    private _getOutPointDirection(index: Number): DIRECTION{
        for(let i = 0; i < FC_Board._ChessPointOutDirArr.length; i++){
            let config = FC_Board._ChessPointOutDirArr[i];
            let start = config[0];
            let end = config[1];
            let dir = config[2];
    ...
    private _checkIsStartFlyPoint(index: Number): Boolean{
        for(let i = 0; i < FC_Board._ChessPointPlaneStartIndexArr.length; i++){
            let temp = FC_Board._ChessPointPlaneStartIndexArr[i];
            if(temp == index){
                return true;
            }
        }
    ...
    private _checkIsCutInPoint(index: Number): Boolean{
        for(let i = 0; i < FC_Board._ChessPointPlaneToInIndexArr.length; i++){
            let temp = FC_Board._ChessPointPlaneToInIndexArr[i];
            if(temp == index){
                return true;
            }
        }
    ...
    private _checkIsFlyPoint(point: FC_ChessPoint){
        for(let i = 0; i < FC_Board._ChessPointPlaneFlyIndexArr.length; i++){
            let temp = FC_Board._ChessPointPlaneFlyIndexArr[i];
            let start = temp[0];
            let end = temp[1];
    ...
    private _checkIsCrossPoint(point: FC_ChessPoint){
        for(let i = 0; i < FC_Board._ChessPointPlaneInCrossFlyLineIndexArr.length; i++){
            let temp = FC_Board._ChessPointPlaneInCrossFlyLineIndexArr[i];
            let p = temp[0];
