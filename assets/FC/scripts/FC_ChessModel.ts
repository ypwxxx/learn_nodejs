...
import { PlayerTypeChessPointArray, PlayerTypeChessPointBase } from "./FC_Interface";
...
export default class FC_ChessModel {
    private constructor() {
        this._pointOutArr = [];
        this._pointInArr = {
            red: [],
    ...
    private static instance: FC_ChessModel = null;
    public static getInstance(): FC_ChessModel{
        this.instance = this.instance || new FC_ChessModel();
    ...
    private _initOuterRing(){
        for(let i = 0; i < FC_ChessModel._ChessPointOutPosArr.length; i++){
            let p = FC_ChessModel._ChessPointOutPosArr[i];       // 坐标
    ...
    private _initInnerRing(){
        for(let i = 0; i < FC_ChessModel._ChessPointInPosArr.length; i++){
            let temp = FC_ChessModel._ChessPointInPosArr[i];
            let dir = FC_ChessModel._ChessPointInDirArr[i];      // 方向
            ...
            for(let j = 0; j < temp.length; j++){
                ...
                }else if(i == PLANE_TYPE.YELLOW){
                    this._pointInArr.yellow.push(point);
                }else if(i == PLANE_TYPE.BLUE){
                    this._pointInArr.blue.push(point);
                }else if(i == PLANE_TYPE.GREEN){
    ...
    private _initWaitPoints(){
        for(let i = 0; i < FC_ChessModel._ChessPointPlaneWaitPosArr.length; i++){
            let p = FC_ChessModel._ChessPointPlaneWaitPosArr[i];     // 坐标
            let dir = FC_ChessModel._ChessPointPlaneWaitDirArr[i];   // 方向
    ...
    private _initStopPoints(){
        for(let i = 0; i < FC_ChessModel._ChessPointPlaneStopPosArr.length; i++){
            let temp = FC_ChessModel._ChessPointPlaneStopPosArr[i];
            let dir = FC_ChessModel._ChessPointPlaneWaitDirArr[i];       // 方向
    ...
    private _getOutPointDirection(index: Number): DIRECTION{
        for(let i = 0; i < FC_ChessModel._ChessPointOutDirArr.length; i++){
            let config = FC_ChessModel._ChessPointOutDirArr[i];
            let start = config[0];
            let end = config[1];
            let dir = config[2];
    ...
    private _checkIsStartFlyPoint(index: Number): Boolean{
        for(let i = 0; i < FC_ChessModel._ChessPointPlaneStartIndexArr.length; i++){
            let temp = FC_ChessModel._ChessPointPlaneStartIndexArr[i];
            if(temp == index){
                return true;
            }
        }
    ...
    private _checkIsCutInPoint(index: Number): Boolean{
        for(let i = 0; i < FC_ChessModel._ChessPointPlaneToInIndexArr.length; i++){
            let temp = FC_ChessModel._ChessPointPlaneToInIndexArr[i];
            if(temp == index){
                return true;
            }
        }
    ...
    private _checkIsFlyPoint(point: FC_ChessPoint){
        for(let i = 0; i < FC_ChessModel._ChessPointPlaneFlyIndexArr.length; i++){
            let temp = FC_ChessModel._ChessPointPlaneFlyIndexArr[i];
            let start = temp[0];
            let end = temp[1];
    ...
    private _checkIsCrossPoint(point: FC_ChessPoint){
        for(let i = 0; i < FC_ChessModel._ChessPointPlaneInCrossFlyLineIndexArr.length; i++){
            let temp = FC_ChessModel._ChessPointPlaneInCrossFlyLineIndexArr[i];
            let p = temp[0];
